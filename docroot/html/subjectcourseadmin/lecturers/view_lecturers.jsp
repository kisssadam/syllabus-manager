...
<aui:form method="post" name="fmLecturer">
	<liferay-ui:search-container delta="<%=delta%>" emptyResultsMessage="lecturers-not-found" iteratorURL="<%=iteratorURL%>" rowChecker="<%= new RowChecker(renderResponse) %>">
		...
		<liferay-ui:search-container-results
			...
			total="<%=LecturerLocalServiceUtil.getLecturersCount()%>"
		/>
		...
		<liferay-ui:search-container-row className="hu.unideb.inf.model.Lecturer" escapedModel="<%= true %>" modelVar="lecturer" keyProperty="lecturerId">
			<c:if test='<%=LecturerPermission.contains(permissionChecker, lecturer.getLecturerId(), "VIEW")%>'>
				...
				<liferay-ui:search-container-column-jsp path="/html/subjectcourseadmin/lecturers/lecturer_actions.jsp" align="right" />
			</c:if>
...
<c:if test='<%=ModelPermission.contains(permissionChecker, scopeGroupId, ActionKeys.DELETE_LECTURERS)%>'>
